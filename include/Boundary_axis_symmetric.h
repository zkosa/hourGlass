...
class Boundary_axis_symmetric : public Boundary {
	...
	std::function<double(double)> contour = std::bind(&Boundary_axis_symmetric::hour_glass_shape, this, std::placeholders::_1);
/*
...
*/
	double distance2(double X, double X0, double R0) const {
		return (X - X0)*(X - X0) + (hour_glass_shape(X) - R0)*(hour_glass_shape(X) - R0);  // fine
	}
	//std::function<double(double, double, double)> distance2_fun = distance2;
	std::function<double(double, double, double)> distance2_fun = std::bind(&Boundary_axis_symmetric::distance2, this, std::placeholders::_1, std::placeholders::_2, std::placeholders::_3);
